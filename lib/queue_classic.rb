module QueueClassicRspec
  def self.find_by_args(queue_name, method, args, table = "queue_classic_jobs")
    q = "SELECT * FROM #{table} WHERE q_name = $1 AND method = $2 AND args = $3"
...
RSpec::Matchers.define :have_queued do |*expected|
  match do |actual|
    ...
    results = QueueClassicRspec.find_by_args(queue_name, method, expected)
    results.size > 0
  end
...
RSpec::Matchers.define :have_scheduled do |*expected_args|
  ...
  match do |actual|
    method = "#{actual.to_s}._perform"
    queue_name = actual.queue.name
    results = QueueClassicRspec.find_by_args(queue_name,
                                             method,
                                             expected,
